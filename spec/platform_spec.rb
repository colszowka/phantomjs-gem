...
describe Phantomjs::Platform do
  ...
  describe "with a system install present" do
    describe "#system_phantomjs_installed?" do
      it "is true when the system version matches Phantomjs.version" do
        expect(Phantomjs::Platform).to receive(:system_phantomjs_version).and_return(Phantomjs.version)
        expect(Phantomjs::Platform.system_phantomjs_installed?).to be_truthy
      end

      it "is false when the system version does not match Phantomjs.version" do
        expect(Phantomjs::Platform).to receive(:system_phantomjs_version).and_return('1.2.3')
        expect(Phantomjs::Platform.system_phantomjs_installed?).to be_falsey
      end

      it "is false when there's no system version" do
        expect(Phantomjs::Platform).to receive(:system_phantomjs_version).and_return(nil)
        expect(Phantomjs::Platform.system_phantomjs_installed?).to be_falsey
      end
    end
  end
  ...
  describe "on a 64 bit linux" do
    ...
    describe "without system install" do
      before(:each) { allow(Phantomjs::Platform).to receive(:system_phantomjs_version).and_return(nil) }
